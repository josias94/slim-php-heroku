...
class Pasajero{
    ...
    public static function Compare($p1, $p2, $campo){
        if($p1->$campo == $p2->$campo){
            return 0;
        }
        return ($p1->$campo > $p2->$campo) ? 1 : -1;
    }
...
class Vuelo{
    ...
    public function Sort($campo, $mayorAMenor = 0){
        for ($i=0; $i < count($this->_listaDePasajeros); $i++){ 
            for ($j=0; $j < count($this->_listaDePasajeros) - 1; $j++){
                if(Pasajero::Compare($this->_listaDePasajeros[$i], $this->_listaDePasajeros[$j], $campo) == -1){
                    $aux = $this->_listaDePasajeros[$i];
                    $this->_listaDePasajeros[$i] = $this->_listaDePasajeros[$j];
                    $this->_listaDePasajeros[$j] = $aux;
                }
            }
        }
        if($mayorAMenor)
        {
            $this->_listaDePasajeros = array_reverse($this->_listaDePasajeros);
        }
    }
}
...
$p3 = new Pasajero("CCCCC", "ccccc", 10000003, true);
